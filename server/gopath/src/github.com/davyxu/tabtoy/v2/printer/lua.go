...
func printTableLua(g *Globals, stream *Stream, tab *model.Table) bool {
	...
	stream.Printf("export var %s : table.I%sDefine[] = [\n", tab.LocalFD.Name,tab.LocalFD.Name)
		...
		if combineStruct.Usage == model.DescriptorUsage_CombineStruct {
			...
			if fd.Complex != nil && len(fd.Complex.Indexes) > 0 {
				...
				for _, key := range fd.Complex.Indexes {
					mapperVarName := fmt.Sprintf("export var %sBy%s : game.Dictionary<table.I%sDefine>", fd.Name, key.Name,fd.Name)

					stream.Printf("\n// %s\n", key.Name)
					stream.Printf("%s = {}\n", mapperVarName)
					stream.Printf("function read%sBy%s(){\n", fd.Name, key.Name)
					stream.Printf("  for(let rec of %s) {\n", fd.Name)
					stream.Printf("    %sBy%s[rec.%s] = rec; \n  }\n", fd.Name, key.Name,key.Name)
					stream.Printf("}\n")
					stream.Printf("read%sBy%s();\n", fd.Name, key.Name)
				}

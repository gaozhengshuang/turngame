...
module {{.Namespace}}{{$globalIndex:=.Indexes}}{{$verticalFields:=.VerticalFields}}
{
	{{range .Enums}}
	...
	{
	{{if .IsCombine}}
		public TableLogger:tabtoy.Logger = new tabtoy.Logger();
	{{end}}
	{{range .Fields}}	
		{{.Comment}}
		{{.TypeCode}} {{.Alias}}
	...
	{{if .IsCombine}}
        ...
        public Get{{.RowName}}By{{.IndexName}}({{.IndexName}}:{{.IndexType}}) : {{.RowType}} 
        {
          
            if ( _{{.RowName}}By{{.IndexName}}[{{.IndexName}}])
            {
                return _{{.RowName}}By{{.IndexName}}[{{.IndexName}}];
            }
	...
	{{end}}
		...
		{{range $.Classes}}
		static _{{.Name}}DeserializeHandler:tabtoy.DeserializeHandler<{{.Name}}>;
		static get {{.Name}}DeserializeHandler():tabtoy.DeserializeHandler<{{.Name}}> 
		{
		...
		public static Deserialize( {{.Name}} ins, tabtoy.DataReader reader )
		{
			{{ if $.GenSerializeCode }}
 			var tag = -1;
			...
			for( var i = 0;i< ins.{{$row.FieldDescriptor.Name}}.length;i++)
			{
				var element = ins.{{$row.FieldDescriptor.Name}}[i];
				{{range $b, $key := .IndexKeys}}
				ins._{{$row.FieldDescriptor.Name}}By{{$key.Name}}[element.{{$key.Name}}] = element;
				{{end}}
			}
			{{end}}
		}{{end}}
...
func (self csharpField) ReadCode() string {
	...
	case model.FieldType_Struct:
		...
		baseType = fmt.Sprintf("interface<%s>", self.Complex.Name)
	...
	if self.IsRepeated {
		return fmt.Sprintf("ins.%s.push( reader.Read%s(%s) );", self.Name, baseType, descHandlerCode)
	} else {
...
func (self csharpField) TypeCode() string {
	...
	if self.IsRepeated {
		return fmt.Sprintf("public %s:Array<%s> = new Array<%s>();", raw, self.Name, raw)
	}

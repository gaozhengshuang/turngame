module game {
    ...
    /**
     * 将一个 value 转换成 一个 已解决的 promise（很少用）
    ...
    function unfoldCore<Seed, Element>(elements:Element[],
        ...
        while (result.next && result.promise.status == game.Status.Resolved) {
    ...
    export interface Promise<Value> extends PromiseState<Value> {
         ...
         * 所有的Promise都解决
         */
        when(promises:Promise<any>[]):Promise<Value>;
    ...
    export interface Deferred<Value> extends PromiseState<Value> {
        /**
         * 按照我的理解：将延递任务实例（Deferred）的另一端（promise）返回出去给外面侦听 resolve 或 reject 事件
         * Returns the encapsulated promise of this deferred instance.
         * The returned promise supports composition but removes the ability to resolve or reject the promise.
         */
        promise():Promise<Value>;

        /**
         * 将返回出去的 promise 标记为解决，并将结果递交出去。
         * Resolve the promise.
         */
        resolve(result:Value):Deferred<Value>;
        /**
         * 将返回出去的 promise 标记为解决，但推迟 指定的毫秒数，才将结果递交出去。
         * Delay Resolve the promise.
         */
        delayResolve(result:Value, delay?:number):Deferred<Value>;
        /**
         * 将返回出去的 promise 标记为拒绝，并递交拒绝的原因。
         * Reject the promise.
         */
    ...
    /**
     * 将所有返回Promise传入，当所有Promise都解决时才会done，如果有一个Promise被拒绝，则会触发fail
        ...
        when(promises:Promise<any>[]):Promise<Value> {
            ...
            promises.forEach((p, i) => {
                p
                    ...
                    .fail(e => {
                        if (this.status !== Status.Rejected)
                            this.deferred.reject(new Error("when: one or more promises were rejected"));
                    });
            }, this);
    ...
    class DeferredI<Value> implements Deferred<Value> {
        ...
        private _resolved:(v:Value) => void = (v:Value) => {
        };
        private _rejected:(err:Rejection) => void = (err:Rejection) => {
        };
        ...
        delayResolve(result:Value, delay?:number) {
            setTimeout(this.resolve, this, delay, result);
            return this;
        }
